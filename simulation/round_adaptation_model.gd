...
func interpret(golfer: Node, context: Dictionary) -> Dictionary:
	...
	var progress := clamp(float(context.get("round_progress", 0.0)), 0.0, 1.0)
	var endurance := _trait_01(golfer, "endurance", 70.0)
	var responsiveness := _trait_01(golfer, "responsiveness_to_experience", 50.0)
	var baseline_confidence := _trait_01(golfer, "confidence", 50.0)
	var recent_average_to_par := float(context.get("recent_average_to_par", 0.0))
	var recent_count := int(context.get("recent_holes_count", 0))
	...
	var endurance_vulnerability := 1.0 - endurance
	var physical_load_exposure := progress * endurance_vulnerability
	...
	var confidence_momentum_signal := recent_form_signal * responsiveness
...
func _trait_01(golfer: Node, property_name: String, fallback: float) -> float:
	var raw = golfer.get(property_name)
	if raw == null:
		raw = fallback
