...
const GROUP_IDS := ["lead", "middle", "follow"]
...
func _run() -> void:
	print("POC-27C DIAGNOSTIC: three foursomes full 18-hole traffic progression")
	...
	assert(runtime.add_group("middle", _foursome(1), "default", -1, 28100))
	assert(runtime.add_group("follow", _foursome(2), "default", -1, 29100))
	...
	print("DIAG INITIAL next_block_reason=%s waiting=%d" % [str(blocked_initial.get("reason", "")), runtime.living_course.start_sequencer.waiting_group_ids().size()])

	var iterations: int = 0
	var previous_holes := {"lead": 1, "middle": 1, "follow": 1}
	var max_live_groups: int = 0
	var max_occupied_holes: int = 0
	var saw_same_hole_overlap: bool = false
	var saw_wait: bool = false
	var released_groups: Dictionary = {"lead": true, "middle": false, "follow": false}
	...
	assert(bool(released_groups["middle"]))
	assert(bool(released_groups["follow"]))
	assert(saw_same_hole_overlap)
	assert(saw_wait)
	assert(max_live_groups >= 3)
	...
	print("POC27C_THREE_GROUP_DIAG_SUMMARY starts=%d finishes=%d waits=%d releases=%d max_live=%d max_holes=%d time=%.1f iterations=%d events=%d" % [starts, finishes, waits, releases, max_live_groups, max_occupied_holes, runtime.current_time_seconds, iterations, runtime.event_history.size()])
	print("POC-27C THREE-GROUP FULL-ROUND DIAGNOSTIC PASSED")
	_cleanup_and_quit(0)
