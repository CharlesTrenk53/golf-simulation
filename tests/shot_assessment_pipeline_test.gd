...
func _init() -> void:
	...
	_expect(float(supportive_result["assessment"]["objective"]["model_success_chance"]) == 62.0, "objective model success remains the supplied shot probability")
	_expect(float(supportive_result["assessment"]["subjective"]["social_readiness"]) > float(intimidating_result["assessment"]["subjective"]["social_readiness"]), "playing group changes social execution readiness")
	_expect(float(supportive_result["assessment"]["willingness"]["willingness_score"]) > float(intimidating_result["assessment"]["willingness"]["willingness_score"]), "supportive group can increase willingness for the same shot")
	_expect(float(supportive_result["assessment"]["subjective"]["believed_success_chance"]) > float(intimidating_result["assessment"]["subjective"]["believed_success_chance"]), "playing group can alter perceived success without rewriting objective success")
	...
	print("Supportive social readiness: %.2f | willingness: %.2f | believed success: %.2f" % [
		float(supportive_result["assessment"]["subjective"]["social_readiness"]),
		float(supportive_result["assessment"]["willingness"]["willingness_score"]),
		float(supportive_result["assessment"]["subjective"]["believed_success_chance"])
	])
	print("Intimidating social readiness: %.2f | willingness: %.2f | believed success: %.2f" % [
		float(intimidating_result["assessment"]["subjective"]["social_readiness"]),
		float(intimidating_result["assessment"]["willingness"]["willingness_score"]),
		float(intimidating_result["assessment"]["subjective"]["believed_success_chance"])
	])
	print("============================================================")
