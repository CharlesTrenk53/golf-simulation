...
func _init() -> void:
	...
	print("Chosen: %s (%s) | quality %s | score %.2f | best %s %.2f | gap %.2f" % [shot["option"], shot["club_name"], shot["decision_quality"], shot["decision_score"], shot["decision_best_option"], shot["decision_best_score"], shot["decision_gap"]])
	print("------------------------------------------------------------")
...
func _print_breakdown(b: Dictionary) -> void:
	print("OPTION %s | club %s | FINAL %.2f | raw %.2f" % [b.get("option", ""), b.get("club", ""), b.get("final_score", 0.0), b.get("raw_score", 0.0)])
	...
	print("  capability %.2f | success %.2f | willingness %.2f | confidence %.2f | miss cost %.2f | base reward %.2f" % [b.get("capability", 0.0), b.get("success_chance", 0.0), b.get("willingness", 0.0), b.get("specific_confidence", 0.0), b.get("miss_cost", 0.0), b.get("base_reward", 0.0)])
	print("  carry: expected %.2f | required %.2f | margin %.2f" % [b.get("expected_carry", 0.0), b.get("required_carry", 0.0), b.get("carry_margin", 0.0)])
	print("  components: capability %.2f | success %.2f | willingness %.2f | confidence %.2f | reward %.2f | miss %.2f | future %.2f | carry %.2f | next %.2f" % [b.get("capability_component", 0.0), b.get("success_component", 0.0), b.get("willingness_component", 0.0), b.get("confidence_component", 0.0), b.get("reward_component", 0.0), b.get("miss_component", 0.0), b.get("future_component", 0.0), b.get("carry_component", 0.0), b.get("next_shot_component", 0.0)])
