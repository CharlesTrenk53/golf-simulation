extends SceneTree

const QuietGolfer = preload("res://tests/quiet_golfer.gd")
const TechniqueSkillDevelopment = preload("res://simulation/technique_skill_development.gd")

const START_AGE := 16
const END_AGE := 76
const SHOTS_PER_YEAR := 1100
const EXECUTION_MEAN := 68.0
const EXECUTION_SD := 6.0
const CHECKPOINTS := [16, 18, 20, 22, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 76]
const OUTPUT_PATH := "res://poc09_aptitude_potential_careers.csv"

const PROFILES := [
	{"id": "fast_low", "label": "Fast learner / lower potential", "aptitude": 1.35, "potential": 55.0},
	{"id": "slow_high", "label": "Slow learner / higher potential", "aptitude": 0.75, "potential": 75.0},
	{"id": "high_high", "label": "High aptitude / high potential", "aptitude": 1.35, "potential": 75.0},
	{"id": "low_low", "label": "Low aptitude / low potential", "aptitude": 0.75, "potential": 55.0},
	{"id": "average", "label": "Average aptitude / average potential", "aptitude": 1.00, "potential": 62.0}
]

var output_rows: Array[String] = []

func _init() -> void:
	print("=== POC-09 APTITUDE X POTENTIAL CAREER DIAGNOSTIC ===")
	print("Same start, age, experience, practice volume, and execution stream; aptitude and latent potential vary.")
	print("This is an acquisition-focused crossover diagnostic: age plasticity applies, but annual age-retention erosion is intentionally omitted.")
	print("profile,label,aptitude,potential,age,effective_skill,delta,potential_resistance")
	output_rows.append("profile,label,aptitude,potential,age,effective_skill,delta,potential_resistance")

	for profile in PROFILES:
		_run_profile(profile)

	_write_output()
	print("=== END POC-09 APTITUDE X POTENTIAL CAREER DIAGNOSTIC ===")
	quit(0)

func _run_profile(profile: Dictionary) -> void:
	var golfer = QuietGolfer.new()
	golfer.profile = 1
	golfer.apply_profile()
	golfer.age = START_AGE
	golfer.driving = 48.0
	golfer.career_shot_experience[0] = 800
	golfer.skill_learning_rates[0] = float(profile["aptitude"])

	var development = TechniqueSkillDevelopment.new()
	development.initialize_from_golfer(golfer)
	development.set_skill_potential(0, float(profile["potential"]))

	# Reset the stream so aptitude and potential are the only between-profile causes
	# of divergence. Every golfer sees the same sequence of execution evidence.
	seed(440921)

	for age in range(START_AGE, END_AGE + 1):
		development.set_current_age(float(age))
		if age in CHECKPOINTS:
			_emit_state(profile, age, development)
		if age == END_AGE:
			break
		for _shot in range(SHOTS_PER_YEAR):
			var persistent_score = clamp(randfn(EXECUTION_MEAN, EXECUTION_SD), 0.0, 100.0)
			development.record_execution(0, persistent_score, 0.0, 0.0, persistent_score)

	golfer.free()

func _emit_state(profile: Dictionary, age: int, development: RefCounted) -> void:
	var state = development.development_state(0)
	var row = "%s,%s,%.2f,%.1f,%d,%.4f,%.4f,%.4f" % [
		str(profile["id"]),
		str(profile["label"]),
		float(profile["aptitude"]),
		float(profile["potential"]),
		age,
		float(state["effective_skill"]),
		float(state["skill_delta"]),
		float(state["potential_resistance"])
	]
	print(row)
	output_rows.append(row)

func _write_output() -> void:
	var output = FileAccess.open(OUTPUT_PATH, FileAccess.WRITE)
	if output == null:
		push_error("Could not write POC-09 aptitude x potential CSV: %s" % OUTPUT_PATH)
		quit(1)
		return
	for row in output_rows:
		output.store_line(row)
	output.close()
	print("Wrote POC-09 aptitude x potential CSV: %s" % OUTPUT_PATH)
